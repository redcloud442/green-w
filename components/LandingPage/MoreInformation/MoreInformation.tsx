...
const MoreInformation = () => {
  ...
  return (
    ...
    >
      <div className="relative flex flex-col items-center lg:items-start justify-start px-20">
      ...
      <div className="relative w-full h-auto lg:h-[40vh] bg-stone-100 p-10 flex flex-col justify-around items-center lg:items-start px-10">
        {moreInformation.map((item, index) => (
          <div className="text-black font-ethnocentric space-y-4" key={index}>
            <h3 className="text-lg lg:text-3xl font-bold w-full max-w-full lg:text-start text-center lg:max-w-4xl tracking-widest border-2 border-cyan-300">
              {item.title}
            </h3>
      ...
      <div className="relative w-full h-auto lg:h-[60vh] px-10 text-center  lg:text-start text-stone-300">
      ...
      <Image
        ...
        height={600}
        quality={100}
        className="-bottom-32 right-20 lg:absolute hidden"
      />
    </section>
  );
};
